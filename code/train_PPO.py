...
class Train_PPO():
    def __init__(self,parameters,train_gui):
        ...
        self.filename = os.path.join(self.output_folder, 'PPO_save-'+datetime.now().strftime("%m.%d.%Y_%H.%M.%S"))
    ...
    def train_PPO(self):
        ...
        """"
        model = PPO.load("results/SAC_save-12.24.2024_18.07.36/final_model.zip",train_env)
        """
        model = PPO('MultiInputPolicy',train_env,
                    n_steps=self.parameters['ctrl_freq']*self.parameters['episode_length'],
                    verbose=1)
